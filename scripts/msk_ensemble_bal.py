...
def create_loader(inputs, targets, batch_size=32): 
	#provide batches of X and y for evaluation
...
def create_unshuffled_loader(inputs, targets, batch_size=32): 
	#provide batches of X and y for evaluation, unshuffled (for comparisons)
	dataset = MyDataset(inputs, targets)
	loader = DataLoader(
		dataset,
		batch_size=batch_size,
...
def process_data(): 
	...
	x_train = pd.read_csv('ft_train_bal' + label + '.csv', sep = ',', squeeze = False, index_col = 0)
	y_train = pd.read_csv('labels_train_bal' + label + '.csv', sep = ',', squeeze = True, index_col = 0)
	x_test = pd.read_csv('ft_test_bal' + label + '.csv', sep = ',', squeeze = False, index_col = 0)
	y_test = pd.read_csv('labels_test_bal' + label + '.csv', sep = ',', squeeze = True, index_col = 0)
...
def softmax_predictive_accuracy(logits_list, y, label):
	...
	np.savetxt('ens_n_preds_bal.csv', n_dif_preds, delimiter = ',')
	...
	np.savetxt('ens_allprobs_bal.csv', probs, delimiter = ',')
	np.savetxt('ens_probs_bal.csv',pred_probs, delimiter = ',')
	np.savetxt('ens_preds_bal.csv',preds, delimiter = ',')
	correct = (pred_class == y)
	np.savetxt('ens_true_bal.csv', y.cpu().data.numpy(), delimiter = ',')
...
if __name__ == "__main__":
	...
	for i in range(1, n_splits+1): 
		#load model
		path_best_model = './mskcl_MLP_best_split_' + str(i) + label + '_bal.pt' #saves the best found model at this path
	...
	torch.save(fold_ensemble, 'ensemble_classifier_update' + label + '_bal.pt')
