...
def process_data_all(n_splits, label): 
	...
	'''
	data_train = pd.read_csv('ft_train_bal' + label + '.csv', sep = ',', squeeze = False, index_col = 0)
	labels_train = pd.read_csv('labels_train_bal' + label + '.csv', sep = ',', squeeze = True, index_col = 0)
	data_test = pd.read_csv('ft_test_bal' + label + '.csv', sep = ',', squeeze = False, index_col = 0)
	labels_test = pd.read_csv('labels_test_bal' + label + '.csv', sep = ',', squeeze = True, index_col = 0)
...
if __name__ == "__main__":
	...
	path_best_model = './mskcl_MLP_best_split_' + str(split+1) + label + '_bal.pt' #saves the best found model at this path
	best_accuracy = 0.0 
	...
	np.save('./mskcl_MLPsplit_' + str(split) + label + '_bal.npy', hyps)
