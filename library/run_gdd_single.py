...
def pred_results(model, data, ctypes):
	...
	return top_preds[::-1], top_probs[::-1], pred_labels, allprobs


def top_shap_single(model, test_x, test_y, colnames):
	#validate shapley values by calculating across all types
	precalc_shap = joblib.load(filename='gddnn_kmeans_output.bz2')
	feature_annotation = pd.read_csv('feature_annotations.csv', index_col = 0)
...
def format_gdd_output(single_data, res, allprobs, shap):
	...
	sv_res = []
	for fv, imp in zip(shap.Shapley_Columns, shap.Shapley_Values):
...
if __name__ == "__main__":
	...
	colnames = pd.read_csv('final_colnames.csv')['columns'].values
	ctypes = list(pd.read_csv('final_tumor_types.csv')['tumor_types'].values)

	#process single instance, save
	gdd_input = process_data_single(single_data, colnames)
	gdd_input_fn = sys.argv[2]
	gdd_input.to_csv(gdd_input_fn)
