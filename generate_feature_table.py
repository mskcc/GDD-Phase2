...
def tumor_mutational_burden(feature_table, maf_somatic):
...
def broad_cna(seg, log_ratio_threshold):
	...
	cytoband_table = pd.read_table('cytoband_table.txt')
...
def fusions(SV):
	...
	fusion_list = pd.read_csv("fusions.txt", sep = '\t')
...
def hotspots(maf):
	#load hotspot list
	hotspot_list = pd.read_csv('final_hotspot_list.csv', index_col = 0)
...
#CHECKED CODE
#set file path
repository_folder = 'msk_solid_heme/' 
output_dir = ''
code_dir = ''
...
#load cancertypes
input_cancertypes = pd.read_table("tumor_type_062520.txt") #need to fix
...
#Signatures
sigs = pd.read_table("msk_solid_heme_data_mutations_unfiltered.sigs.tab.txt")
...
SV = SV.rename({'Tumor_Sample_Barcode':'SAMPLE_ID'}, axis = 1)
...
impact_genes = pd.read_excel('IMPACT505_Gene_list_detailed.xlsx', sheet_name = '505 genes')
impact_genes.columns = ['Hugo_Symbol', 'Name', 'N_exons', 'Panel']
alt_names_list = pd.read_excel('IMPACT505_Gene_list_detailed.xlsx', sheet_name = 'alt_names')
alt_names = {}
...
#remove low purity
feature_table = purity_est(feature_table, maf_somatic, seg)
#mutations
...
for col in feature_table.columns:
	if sum(feature_table[col].values!=0)==0:
		zero_feats.append(col)
zero_feats = pd.DataFrame(zero_feats)
zero_feats.to_csv('zero_features.csv')
feature_table = feature_table[[col for col in feature_table.columns if col not in zero_feats]]

#save feature table
#TODO add sys label
label = str(sys.argv[-1])
file_path = label if '.py' not in label else 'msk_solid_heme_ft'
feature_table.to_csv(file_path + '.csv')
